    ...
    def __new__(cls, name, bases, attrs):
        ...
        for k, v in attrs.items():
            if isinstance(v, Field):
                ...
                if v.primary_key:
                    if primary_key:
                        raise KeyError('Duplicate primary key for fields: %s' % k)
                    primary_key = k
                else:
                    fields.append(k)
        if not primary_key:
...
class Model(dict, metaclass=ModelMetaclass):
    ...
    async def find_all(cls, where=None, args=None, **kwargs):
        ...
        if limit is not None:
            ...
            if isinstance(limit, int):
                ...
                raise ValueError('Invaild limit values: %s' % limit)
    ...
    async def find(cls, pk):
        ret = await select('%s where `%s`=?' % (cls.__select__, cls.__primary_key__), [pk], 1)
        if len(ret) == 0:
            return None
        return cls(**ret[0])
    ...
    async def update(self):
        args = list(map(self.get_value, self.fields))
    ...
    async def remove(self):
        ...
        if rows != 1:
            logging.warning('failed to delete by primary_key: affected rows: %s' % rows)
